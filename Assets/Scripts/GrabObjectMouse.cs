...
public class GrabObjectMouse : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Input.GetMouseButton(0)) //(touch.phase != TouchPhase.Canceled) // Moved나 Stationary인 경우
        {
            if (isGrab) // grab하고있는 object가 있을 때만 회전 수행
            {
                ...
                if (Quaternion.Angle(Quaternion.LookRotation(vStart), Quaternion.LookRotation(vEnd)) > 1) // 한 프레임에서 5도 보다 크게 회전하면 적용
                {
